...
auto to_sqlstr(Driver& driver, const cxxql::expr::create_table_t<Tab>& expr) {
  return to_sql::create_table(driver, expr.table, 
    expr::get_table_name(expr.table),
    avalon::tuple::trans(
      expr::get_table_cols(expr.table),
      [&](const auto& col_design) {
        return to_sql::create_table_col(
          driver, expr.table,
          expr::get_col_name(col_design),
          to_sql::col_type(col_design),
          col_design
        );
      }
    )
